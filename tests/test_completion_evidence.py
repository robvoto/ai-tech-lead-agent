...
def _repo_with_change(tmp_path: Path) -> tuple[Path, str]:
    root = tmp_path / "repo"
    root.mkdir()
    _git(root, "init", "-q")
...
def test_captures_diff_and_passing_validation(tmp_path: Path) -> None:
    root, base_sha = _repo_with_change(tmp_path)
...
def test_out_of_scope_paths_are_flagged(tmp_path: Path) -> None:
    root, base_sha = _repo_with_change(tmp_path)

    evidence = capture_completion_evidence(
        checkout_root=root,
        base_sha=base_sha,
        changed_files=("app.py", "billing/invoices.py"),
        validation_command="",
        relevance_text="update the app.py rendering behaviour",
        timeout_seconds=30,
    )

    assert "billing/invoices.py" in evidence.out_of_scope_paths
    assert "app.py" not in evidence.out_of_scope_paths
