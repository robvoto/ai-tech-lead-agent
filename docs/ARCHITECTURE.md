...
- ATL-039: completion is decided from the AI Tech Lead's own evidence, not the coding agent's success prose. Before the handoff, `5e_discover_validation_command` (`validation_command_discovery.py`) finds the target project's canonical validation command — first from the project's own entry points (`AGENTS.md`, `docs/INDEX.md` one-hop refs, `.agents/skills` best skill), otherwise from exactly one unambiguous repo signal (pytest / `npm test` / `make test` / tox / nox); zero or conflicting signals yield nothing. After the agent reports success, `6b1_capture_validation_evidence` (`completion_evidence.py`) runs in the same checkout the agent used (the request-owned worktree when the Git lifecycle is active, else the resolved project root), builds a `git diff` summary against the task base, runs that canonical command itself (`shell=False`), and lists changed files unrelated to the approved task. `completion_verifier.py` then resolves to exactly one of four outcomes: **Human verification required** when ATL had no command or could not run one (`validation_passed=None`, no LLM call); **Correction required** when ATL's run failed (one bounded cycle, then **Failed**); otherwise the verifier LLM weighs the real diff, validation output and out-of-scope list against the task/plan/acceptance criteria for **Complete** / Correction required / Human verification required. The coding agent still runs the command for its own feedback, but its report is context only.
