...
- ATL-093: for meaningful or risky work, a second, genuinely different coding agent independently reviews the change before completion. `6b2_implementation_review` (`implementation_review.py`) runs after `6b1_capture_validation_evidence`. The review is required when the plan-review `coding_agent_tier` is `standard`/`deep`, the `risk_level` is `MEDIUM`/`HIGH`/`UNKNOWN`, or the task/diff hits a configured `implementation_review_risk_keywords` term (security / auth / migration / database / production / payment …); it is skipped only for `light`, `LOW`, keyword-free changes (and when coding execution or `implementation_review_enabled` is off). The reviewer is the configured `review_agent_command` (distinct from `coding_agent_command`), dispatched through the same `run_coding_agent` path with the operator's `review_agent_args`; those args must make the reviewer read-only for its backend (Codex `-s read-only`, Claude Code `--permission-mode plan`), and the post-run mutation check is the hard backstop — any file the reviewer changes on disk voids the review. It returns `FINDING [required|advisory] …` lines plus a `REVIEW: pass|changes-required` verdict; the verdict and the findings must agree — a `pass` with any `required` finding, or `changes-required` with none, is an unreliable review and fails closed. Required findings drive one bounded correction cycle (back through `5_create_agent_instruction` → `6b1` → `6b2`); a review that is unavailable (no distinct reviewer, or preflight failure), failed/unparseable/self-contradictory, mutated the checkout, or still has required findings after that one cycle is turned by `completion_verifier.py` into **Human verification required** — never Complete. A clean review is positive context for the verifier LLM but never on its own forces a non-Complete outcome. Agent A never reviews or approves its own work. `review_status` is recorded in the ATL-038 audit receipt and the subprocess `review` field.
