...
Keep the model configurable through settings/admin rather than hardcoding it into the risk-review code.
...
Workflow logs should clearly separate each LangGraph node and decision. Use
visible separators and labels such as `NODE` or `DECISION` so the human can
follow the graph flow while watching the terminal. Risk-review logs should show
the AI channel and model, and coding-agent logs should show whether execution
is enabled, the command name, and whether token/cost tracking is available.
