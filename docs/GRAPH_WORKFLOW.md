...
- Before calling the LLM, `2 Analyse Task` also runs bounded project-guidance discovery (`project_guidance_discovery.py`, gated by `project_guidance_discovery_enabled`, on by default): a local, non-LLM read of the resolved target root's own `AGENTS.md`, `docs/INDEX.md`, and `.skills/INDEX.md` — the same three files the project-pack bootstrap treats as canonical — plus, when `.skills/INDEX.md` exists, the single best-scoring skill file (scored against its own index entry, not its full content, so nothing beyond the index itself needs to be opened to pick it). Every file is optional; a target with no project pack yet yields no guidance and analysis proceeds on AI Tech Lead's own core prompt exactly as before. Since this is local file I/O with no LLM call, running it twice on the common no-research-gap path (see below) is not a real cost.
